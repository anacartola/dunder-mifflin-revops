...
> 本页面为作者本人翻译；如有措词问题，欢迎通过 issue 指正。
...
*这个练习自己先做一遍更有意思 —— 完整过程见 [`notebooks/revops_analysis.ipynb`](notebooks/revops_analysis.ipynb)。*

<details>
<summary><b>执行摘要 —— 点击展开</b></summary>
...
**Q3 —— “以当前管道来看，下个季度能达标吗？” → 签约看起来没问题；但本次导出的管道信号不可信。**
2025 年 Q2 的签约进度领先（预计约 **$1.70M**，对比**重建后**的 **$1.17M** 目标（见 Q1），**+46%**）。在建管道在最后四周*看似*下跌 **63%**（$8.46M → $3.12M）—— 但这恰好与导出窗口的结束重合：每周快照覆盖的交易数从约 130 降至 **95 笔**，且**最后约 3 周没有任何新交易进入**。这个“崩塌”在很大程度上是**数据被截断的产物**，而非已证实的下滑。诚实的结论：签约保持节奏；但临近数据截止时，领先指标无法解读 —— 这是数据无法完全支撑的第二个信号。
...
每次 push，CI 都会在干净的运行器上完整执行该 notebook（上方的 **CI** 徽章）—— 因此“下载即可在任何机器运行”是被自动验证的，而不只是承诺。
...
本仓库中的**代码**依据 [MIT 许可证](LICENSE) 发布；**数据集与文字内容**依据 [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) 发布 —— 可自由分享与改编，需署名。
